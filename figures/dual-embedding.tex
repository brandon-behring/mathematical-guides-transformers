...
% SAME box X-hat in R^{n x d}: modality is discharged at this one linear
% interface; position is then added (X = X-hat + P) before the core.
...
\begin{tikzpicture}[
  ...
  \node[box, fill=black!8, minimum height=24mm, right=24mm of $(txtemb)!0.5!(imgemb)$]
    (X) {$\hat{\mathbf X} \in \mathbb{R}^{n\times d}$\\[2pt]\scriptsize position is added next:\\\scriptsize $\mathbf X = \hat{\mathbf X} + \mathbf P$};
