...
> restructure**, and the two ideas here are **split**: **Mixture-of-Experts** becomes its own chapter
> ending **Part 4 (Efficient & Conditional Computation)**, while the **Sparse & Sub-Quadratic Attention**
> chapter (§§1–4 below) **opens Part 5 (Sub-Quadratic & Selective Sequence Models)**, adjacent to the
> Selective-SSM/hybrid arc — NOT paired with MoE, and NOT a standalone chapter "after ch12". The note's
> §5 (KV sparsity + MLA) is **redistributed** into ch05 (MLA on a new `d_cache` symbol) and ch12 (lossy
> eviction remarks), not kept here. BigBird universality is a **parallel** result to `thm-tf7-universal-
> approx`, not a "refinement". IDs become **semantic/chapter-free**. The `ch03/05/09` hooks below use the
> pre-arc numbering — current labels are `prop-tf5-kv-cache` (ch05), `prop-tf7-complexity` (ch07),
> `def-tf12-moe` (ch12). The design detail below (formalizability triage, figures, dossier→section map)
> remains useful and is folded into the plan doc.
>
> _(Original 2026-07-09 status:)_ ideas + resources for the book rewrite; every quantitative claim
> *to-be-anchored* against a primary at authoring time.
